...
export const ArticleDetailModal: React.FC<ArticleDetailModalProps> = ({
  ...
  // Safely check author subdomain
  const authorSubdomain = typeof article.author === 'object' && article.author ? article.author.subdomain : undefined;
  const authorName = typeof article.author === 'object' && article.author ? article.author.name : (article.author || 'Author');
  const authorRole = typeof article.author === 'object' && article.author ? article.author.role : 'Syndicate Contributor';
  const authorAvatar = (typeof article.author === 'object' && article.author?.avatar) || 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=400&auto=format&fit=crop';
  const isVerifiedFounder = typeof article.author === 'object' && !!article.author?.isVerifiedFounder;

  const memberAuthor = authorSubdomain && members && Array.isArray(members)
    ? members.find((m) => m.subdomain === authorSubdomain)
    : undefined;
  ...
  const handleShare = () => {
    if (navigator.clipboard) {
      navigator.clipboard.writeText(`https://${customDomain}/news/${article.slug || article.id}`);
      setCopied(true);
            ...
            <div className="bg-gradient-to-r from-neutral-900 to-neutral-950 border border-neutral-800 rounded-xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-400 mt-0.5">{authorRole}</p>
                  {authorSubdomain && (
                    <p className="text-xs text-amber-400/90 font-mono mt-1">
                      Subdomain: {authorSubdomain}.{customDomain}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3 w-full sm:w-auto">
                {authorSubdomain && onOpenSubdomain && (
                  <button
                    onClick={() => {
                      onClose();
                      onOpenSubdomain(authorSubdomain);
                    }}
